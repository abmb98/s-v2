  ...
  const handleExport = () => {
    const exportData = {
      'Vue d\'ensemble': [
        ...
        ['Score d\'efficacité', `${statistics.efficiencyScore}%`]
      ],
      'Répartition par âge': [
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          ...
          <Badge variant={statistics.efficiencyScore > 80 ? "default" : "secondary"} className="text-sm">
            Score d'efficacité: {statistics.efficiencyScore}%
          </Badge>
        </div>
      </div>
                  ...
                  <div className="relative w-24 h-24 mx-auto mb-4">
                    <svg className="w-24 h-24 transform -rotate-90" viewBox="0 0 100 100">
                      ...
                      <circle 
                        ...
                        cy="50" 
                        r="40" 
                        stroke={statistics.efficiencyScore > 80 ? "#10b981" : statistics.efficiencyScore > 60 ? "#3b82f6" : "#f59e0b"} 
                        strokeWidth="8" 
                        fill="none"
                        strokeDasharray={`${2 * Math.PI * 40}`}
                        strokeDashoffset={`${2 * Math.PI * 40 * (1 - statistics.efficiencyScore / 100)}`}
                        className="transition-all duration-1000"
                      />
                    </svg>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-2xl font-bold text-gray-900">{statistics.efficiencyScore}%</span>
                    </div>
                  </div>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span>Rétention:</span>
                      <span className="font-medium">{statistics.retentionRate}%</span>
                    </div>
                    <div className="flex justify-between">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {statistics.efficiencyScore > 85 && (
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                <div className="flex items-center mb-2">
                  <CheckCircle className="h-5 w-5 text-green-600 mr-2" />
                  <span className="font-medium text-green-900">Performance Excellente</span>
                </div>
                <p className="text-sm text-green-800">
                  Score d'efficacité de {statistics.efficiencyScore}%. Système bien optimisé.
                </p>
              </div>
            )}
          </div>
